...
impl Vm {
    ...
    fn read_param(&mut self, mode_flag: isize) -> Option<Mode> {
        ...
        match mode_flag {
            ...
            _ => panic!("invalid instruction decoding!"),
        }
    }
    ...
    fn opcode(&mut self) -> Option<Opcode> {
        ...
        match op {
            ...
            9 => Some(Opcode::Halt),
            _ => panic!("invalid instruction decoding! {} @ {}", instr, self.ip),
        }
    }
...
mod test {
    ...
    fn jump_test() {
        ...
        let vm = ex.parse::<Vm>().unwrap();
        assert_eq!(vm.run(7), Some(999));
        let vm = ex.parse::<Vm>().unwrap();
        assert_eq!(vm.run(8), Some(1000));
        let vm = ex.parse::<Vm>().unwrap();
        assert_eq!(vm.run(9), Some(1001));
    }
}
